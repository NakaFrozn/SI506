    ...
    for episode in episodes:
        for key, value in episode.items():
            ...
            if key in ("series_season_num", "series_episode_num", "season_episode_num"):
                ...
                episode[key] = utl.to_list(value, delimiter=", ")
    return episodes
...
def get_most_viewed_episode(episodes):
    ...
    for episode in episodes:
        if has_viewer_data(episode):
            if episode["episode_us_viewers_mm"] > most_view:
                most_viewed.clear()
                most_viewed.append(episode)
                most_view = episode["episode_us_viewers_mm"]
            elif episode["episode_us_viewers_mm"] == most_view:
                most_viewed.append(episode)
    return most_viewed
...
def get_news_desks(articles, none_values):
    ...
    for article in articles:
        ...
        # print(news_desk)
        if news_desk:
            if news_desk not in set(news):
                news.append(news_desk)
    return sorted(news)
...
def main():
    ...
    wookie_haruun_kal = utl.get_nested_dict(wookiee_planets, "system", "Al'Har system")
    utl.write_json("stu-wookiee_haruun_kal.json", wookie_haruun_kal)
    # 3.9 CHALLENGE 09
